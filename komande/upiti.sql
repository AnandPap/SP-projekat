...
SELECT dt.YEAR AS Godina,
       ...
       CAST(SUM(ofc.QUANTITY * dp.PROFIT_MARGIN) AS INTEGER) AS "Ukupni prihodi"
FROM ORDER_FACT ofc
...
SELECT dp.CATEGORY_NAME AS "Ime kategorije",
       ...
       CAST(SUM(ofc.QUANTITY * dp.PROFIT_MARGIN) AS INTEGER) AS "Ukupni profit",
       CAST(SUM(ofc.QUANTITY * dp.PROFIT_MARGIN)/NULLIF(SUM(ofc.QUANTITY), 0) AS INTEGER) AS "Zarada po jedinici proizvoda"
FROM ORDER_FACT ofc
JOIN DIM_PRODUCT dp ON ofc.DIM_PRODUCT_FK = dp.DIM_PRODUCT_ID
...
SELECT dc.COMPANY_NAME AS "Ime kompanije kupca",
       CAST(SUM(ofc.QUANTITY * dp.LIST_PRICE) AS INTEGER) AS "Ukupna potrošnja kompanije kupca",
       CAST(SUM(dp.PROFIT_MARGIN * ofc.QUANTITY) AS INTEGER) AS "Naš ukupni profit poslovanja s njima"
FROM ORDER_FACT ofc
...
SELECT de.FULL_NAME AS Ime,
       ...
       CAST(SUM(ofc.QUANTITY * dp.LIST_PRICE) AS INTEGER) AS "Ukupna vrijednost dogovorenih narudžbi"
FROM ORDER_FACT ofc
...
SELECT dt.YEAR AS Godina,
       ...
       CAST(SUM(ofc.QUANTITY * dp.PROFIT_MARGIN) AS INTEGER) AS Profit
FROM ORDER_FACT ofc
JOIN DIM_TIME dt ON ofc.ORDER_DATE_FK = dt.ORDER_DATE_ID
JOIN DIM_PRODUCT dp ON ofc.DIM_PRODUCT_FK = dp.DIM_PRODUCT_ID
...
SELECT dw.CITY_NAME,
       CAST(SUM(ifc.QUANTITY * dp.STANDARD_COST) AS INTEGER) AS "Ukupna vrijednost",
       CAST(SUM(ifc.QUANTITY * dp.PROFIT_MARGIN) AS INTEGER) AS "Potencijalni profit"
FROM INVENTORY_FACT ifc
JOIN DIM_WAREHOUSE dw ON ifc.DIM_WAREHOUSE_FK = dw.DIM_WAREHOUSE_ID
JOIN DIM_PRODUCT dp ON ifc.DIM_PRODUCT_FK = dp.DIM_PRODUCT_ID
